        ...
        public void Start()
        {
            if (Proces == ProcesType.Dead)
            {
                ...
                else
                {
                    ...
                    {
                        client.Player.MessageBox("", new Action<Client.GameClient>(p => p.Teleport(430, 251, 1002, 0)), null, 60, MsgServer.MsgStaticMessage.Messages.ElitePKTournament);
                    }
                }
            }
        }
                ...
                if (tournament.GetReward(client, out Rank))
                {
                    ...
                    if (tournament.GroupTyp == GroupTyp.EPK_Lvl130Plus)
                    {
                        if (Rank == 1)
                        {
                            ...
                            client.CreateBoxDialog("You've received a 50,000 ConquerPoints and rare garement/accessories box.");

                        }
                        else if (Rank == 2)
                        {
                            client.Player.ConquerPoints += 25000;
                            client.CreateBoxDialog("You've received a 25,000 ConquerPoints.");
                        }
                        else if (Rank == 3)
                        {
                            client.Player.ConquerPoints += 15000;
                            client.CreateBoxDialog("You've received a 15,000 ConquerPoints.");

                        }
                        else
                        {
                            client.Player.ConquerPoints += 5000;
                            client.CreateBoxDialog("You've received a 5,000 ConquerPoints.");
                        }
                    }
                    return true;
                }
