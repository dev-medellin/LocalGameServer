                        ...
                        case ActionID.Request:
                            {
                                using (var rec = new ServerSockets.RecycledPacket())
                                {
                                    ...
                                    {
                                        user.CreateBoxDialog("Sorry you used the maximum available chance");
                                        break;
                                    }
                                    if ((Name != null && Name != "" && Name.Length >= 4 && Program.NameStrCheck(Name) && user.Player.NameEditCount < Database.Server.NameChangeCount(user.Player.VipLevel) && !Database.Server.NameUsed.Contains<int>(Name.GetHashCode())) || obj.Bypass)
                                    {
