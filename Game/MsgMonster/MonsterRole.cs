        ...
        public void Dead(ServerSockets.Packet stream, Client.GameClient killer, uint aUID, Role.GameMap GameMap, bool CounterKill = false)
        {
            if (Alive)
            {
                ...
                if (killer != null && killer.Player != null && !CounterKill)
                {
                    ...
                    {
                        killer.SendSysMesage("You`ve killed " + killer.TotalSouls + " so far, when you reach 300,000, you`ll get 20 PVE Points automatically!", MsgMessage.ChatMode.TopLeft);
                    }
                    ...
                    {
                        DropItemID(killer, 720549, stream);

                    }

                    if (killer.DBMobs > killer.MaxDBMobs)
                    {
                        killer.DBMobs = 0;
                        ushort xx = (ushort)(X + Role.Core.Random.Next(1, 3));
                        ushort yy = (ushort)(Y + Role.Core.Random.Next(1, 3));
                        if (killer.Map.AddGroundItem(ref xx, ref yy))
                            DropItem(stream, killer.Player.UID, killer.Map, 1088000, xx, yy, MsgFloorItem.MsgItem.ItemType.Item, 0, true, 0, killer);
                        killer.MaxDBMobs = Role.Core.Random.Next(300, 600);

                    }
                    if (killer.Player.VipLevel == 6)
                    {
                        if (Role.MyMath.Success(15.0))
                        {
                            killer.Player.ConquerPoints += val;
                            killer.SendSysMesage("You got " + val + " ConquerPoints!", MsgMessage.ChatMode.TopLeft);
                            Program.CPsHuntedSinceRestart += val;
                        }
                    }
                    else if (Role.MyMath.Success(12.0))
                    {
                        killer.Player.ConquerPoints += val;
                        killer.SendSysMesage("You got " + val + " ConquerPoints!", MsgMessage.ChatMode.TopLeft);
                        Program.CPsHuntedSinceRestart += val;
                    ...
                    if (Role.Core.Rate(2))
                    {
                        Program.ExpBallsDropped++;
                        if (killer.Player.VipLevel >= 5 && killer.Inventory.HaveSpace(1) && killer.Player.LootExpBall)
                        {
                            killer.Inventory.AddItemWitchStack(722136, 0, 1, stream);
                            if (killer.Inventory.Contain(722136, 30) && killer.Player.VipLevel == 6)
                            {
                                killer.Inventory.Remove(722136, 30, stream);
                                killer.Inventory.Add(stream, 720757, 1);
                                killer.SendSysMesage("[VIP-6] Exp balls got autopacked.", MsgMessage.ChatMode.TopLeft);
                            }
                            killer.SendSysMesage("You got an EXP Ball in your inventory.", MsgMessage.ChatMode.TopLeft);
                        }
                        else
                            DropItemID(killer, 722136, stream);
                    }
 ...
 killer.Player.QuestGUI.SendAutoPatcher("Congratulations!~You~found~an~invitation~to~the~Treasure~Appraisal~Meeting.~Open~your~inventory~and~read~it.", ActiveQuest.FinishNpcId.Map, ActiveQuest.FinishNpcId.X, ActiveQuest.FinishNpcId.Y, ActiveQuest.FinishNpcId.ID);
    ...
    killer.Player.QuestGUI.SendAutoPatcher("You`ve~collected~the~bags~of~10~Bandits.~Hurry~to~report~back~to~the~ForestGuvernor!", ActiveQuest.FinishNpcId.Map, ActiveQuest.FinishNpcId.X, ActiveQuest.FinishNpcId.Y, ActiveQuest.FinishNpcId.ID);
                ...
                if (killer != null)
                {
                    ...
                    if (Boss > 0 && (Family.ID == 20070 || Family.ID == 20160))//Snow Banshee
                    {
                        const ushort GetStudyPoints = 500;

                        List<uint> DropIems = Family.ItemGenerator.GenerateSoulsItems(0);
                        foreach (var ids in DropIems)
                        {
                            ...
                            if (killer.Inventory.HaveSpace(1))
                            {
                                killer.Inventory.Add(DataItem, DBItem, stream);
                            }
                            else
                            {
                                if (killer.Map.AddGroundItem(ref xx, ref yy, 3))
                        ...
                        if (Family.ID == 20160)
                            this.DropItem(stream, killer.Player.UID, killer.Map, 723094, (ushort)Program.GetRandom.Next(X - 7, X + 7), (ushort)Program.GetRandom.Next(Y - 7, Y + 7), MsgFloorItem.MsgItem.ItemType.Item, 0, false, 0);
                        else
                        {
                            for (int i = 0; i < 3; i++)
                                DropItem(stream, killer.Player.UID, killer.Map, 723744, (ushort)Program.GetRandom.Next(X - 7, X + 7), (ushort)Program.GetRandom.Next(Y - 7, Y + 7), MsgFloorItem.MsgItem.ItemType.Item, 0, false, 0);
                        }
                        if (killer.Team != null)
                        {
                            foreach (var member in killer.Team.Temates)
                    ...
                    if (Boss > 0 && (Family.ID == 20060 || Family.ID == 4145))//terato dragon
                    {
                        const ushort GetStudyPoints = 50;
                        ...
                        List<uint> DropIems2 = Family.ItemGenerator.GenerateSoulsItems(0);
                        foreach (var ids in DropIems2)
                        {
                            ...
                            ushort xx = X;
                            ushort yy = Y;
                            if (killer.Inventory.HaveSpace(1) && (killer.Player.VipLevel == 6 || Family.ID == 4145))
                            {
                                killer.Inventory.Add(DataItem, DBItem, stream);
                            }
                            else if (killer.Map.AddGroundItem(ref xx, ref yy, 3))
                            {
                                MsgFloorItem.MsgItem DropItem = new MsgFloorItem.MsgItem(DataItem, xx, yy, MsgFloorItem.MsgItem.ItemType.Item, 0, DynamicID, Map, killer.Player.UID, true, GMap);
                                if (killer.Map.EnqueueItem(DropItem))
                                {
                    ...
                    {
                        MsgSchedules.LavaBeast.Add(new coords(Family.LavaX, Family.LavaY) { });
                        MsgSchedules.NextLavaBeast = DateTime.Now.AddMinutes(3);
                        MsgSchedules.LavaBeastsCount++;
                        const ushort GetStudyPoints = 20;
                        List<uint> DropIems = new List<uint>();


                        if (Role.Core.Rate(2.5))
                        {
                            DropIems = Family.ItemGenerator.GenerateSoulsItems(3);
                        }
                        if (Role.Core.Rate(5))
                            DropIems.Add(720399);
                        else DropIems.Add(720549);
                        foreach (var ids in DropIems)
                        {
                            MsgServer.MsgGameItem DataItem = new MsgServer.MsgGameItem();
                            DataItem.ITEM_ID = ids;
                            ...
                            ushort xx = X;
                            ushort yy = Y;
                            if (killer.Inventory.HaveSpace(1) && killer.Player.VipLevel == 6)
                            {
                                killer.Inventory.Add(DataItem, DBItem, stream);
                            }
                            else if (killer.Map.AddGroundItem(ref xx, ref yy, 3))
                        ...
                        DropItem(stream, killer.Player.UID, Database.Server.ServerMaps[Map], 721003, (ushort)(X + Program.GetRandom.Next(-1, 2)), (ushort)(Y + Program.GetRandom.Next(-1, 2)), MsgItem.ItemType.Item, 1, false, 0);
                        //if (Role.Core.Rate(0.001))
                        //{
                        //    var items = new uint[] { 422000, 422010, 422020, 422030, 422040 };
                        //    var item = items[Program.GetRandom.Next(0, items.Length)];
                        //    DropItem(stream, killer.Player.UID, Database.Server.ServerMaps[Map], item, (ushort)(X + Program.GetRandom.Next(-3, 4)), (ushort)(Y + Program.GetRandom.Next(-3, 4)), MsgItem.ItemType.Item, 1, false, 0);
                        //}
                        if (killer.Team != null)
                        {
                            foreach (var member in killer.Team.Temates)
                    ...
                    if ((Family.Settings & MonsterSettings.DropItemsOnDeath) == MonsterSettings.DropItemsOnDeath)
                    {
                        ...
                        /* if (rand > 300 && rand < 340 && killer.Player.Map != 1700)//360
                         {
                             ushort xx = X;
                             ushort yy = Y;
                             if (killer.Player.MyRandom.Next(1, 100) < 20)
                             {
                                 var drops = Drops2.ToArray();
                                 var rnd = Role.Core.Random.Next(drops.Length);
                                 //var rnd = killer.Player.MyRandom.Next(1, 100) < drops.Length);
                                 var drop = drops[rnd];
                                 if (killer.Player.MyRandom.Next(1, 100) < drop.Rate)
                                 {
                                     if (killer.Player.VipLevel < 4)
                                         DropItemID(killer, drop.Item, stream);
                                     else
                                         DropCps(killer, drop.Item, stream);
                                 }
                             }
                             else
                             {
                        ...
                        else if (rand > 700)//&& rand < 770)
                        {
                            ...
                            for (byte i = 0; i < count; i++)
                            {
                                ...
                                {
                                    bool drop = true;
                                    if (ID == 723341 && killer != null)
                                        if (killer.Player.VipLevel == 6 && killer.LootEnduranceBooks)
                                        {
                                            if (killer.Inventory.HaveSpace(1))
                                            {
                                                killer.Inventory.AddItemWitchStack(723341, 0, 1, stream);
                                                drop = false;
                                            }
                                            else
                                                killer.SendSysMesage("An Endurance book dropped on the floor!");
                                        }
        ...
        public void DropItemID(Client.GameClient killer, uint itemid, ServerSockets.Packet stream, byte range = 3)
        {
            ...
            if (killer.Map.AddGroundItem(ref xx, ref yy, range))
            {
                    ...
                    if (itemid == 1088000 || itemid == 720891 || itemid == 727100)
                    {
                        ...
                        if (killer.Player.VipLevel == 6)
                        {
                            if (killer.Inventory.Contain(1088000, 10))
                            {
                                ...
                                killer.SendSysMesage("[VIP-6] DragonBalls autopacked.");
                            }
                        }
                ...
                if (itemid == 720549)
                {
                    if (killer.Player.VipLevel == 6 && killer.Inventory.HaveSpace(1) && killer.LootRefenieryPacks)
                    {
                        drop = false;
                        killer.Inventory.Add(stream, itemid, 1);
                        killer.SendSysMesage("A Refinary pack has been autolooted!", MsgMessage.ChatMode.Talk);
                    }
                    else
        ...
        public MonsterRole(MonsterFamily Famil, uint _UID, string locationspawn, Role.GameMap _map)
        {
            ...
            State = MobStatus.Idle;
            BitVector = new Role.StatusFlagsBigVector32(32 * 4);//5
            Boss = Family.Boss;
        ...
        public unsafe ServerSockets.Packet GetArray(ServerSockets.Packet stream, bool view)
        {
            ...
            stream.Write(Mesh);
            stream.Write(UID);//8
            stream.ZeroFill(10);//12

            for (int x = 0; x < BitVector.bits.Length; x++)
                stream.Write(BitVector.bits[x]);//22



            stream.ZeroFill(36 + 2 + 4);//30
            ...
            if (Boss > 0)
            {
                ...
                {
                    uint key = (ushort)(Family.MaxHealth / 10000);
                    if (key != 0)
                        stream.Write((ushort)(HitPoints / key));//72
                    else
            ...
            stream.Write((byte)Action);//85
            stream.ZeroFill(89);//86

            stream.Write((byte)Boss);//175 
            stream.ZeroFill(35 + 1);//176
        ...
        bool ConfirmBoss()
        {
            if (Family.ID == 20070 || Family.ID == 20160 || Family.ID == 4145) return true;
            return false;
        }
