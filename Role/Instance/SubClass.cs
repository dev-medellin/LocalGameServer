...
namespace COServer.Role.Instance
{
    public class SubClass
    {
        ...
        {
            foreach (var it in src.Values)
            {
                switch (it.ID)
                {
                    case Database.DBLevExp.Sort.Wrangler:
                        client.Status.MaxHitpoints += Database.Server.SubClassInfo.GetDamage(it.ID, it.Phrase)[0];
                        break;
                }
            }
            var item = src.Values.Where(x => x.ID == client.Player.ActiveSublass).FirstOrDefault();
            if (item != null)
            {
                switch (item.ID)
                {
