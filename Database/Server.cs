...
namespace COServer.Database
{
    public class Server
    {
        ...
        public static void Initialize()
        {
            ...
            //Program.TestServer = IniFile.ReadString("ServerInfo", "Test", "NO") == "Yes";
            //Console.WriteLine($"Test-Server Mode: {Program.TestServer}", ConsoleColor.Yellow);
            Program.ServerConfig.IPAddres = IniFile.ReadString("ServerInfo", "AddresIP", "25.88.110.217");
            ...
            Program.ServerConfig.InterServerAddress = IniFile.ReadString("InterServer", "AddresIP", "25.88.110.217");
            Program.ServerConfig.InterServerPort = IniFile.ReadUInt16("InterServer", "Port", 9959);
            ...
            if (Program.DeleteItems)
            {
                //Program.ConsoleCMD("reset");
            }
                ...
                using (var reader = new StreamReader(Program.ServerConfig.DbLocation + "\\Spawns.txt"))
                {
                    ...
                    foreach (var line in values)
                    {
                        ...
                        if (colletion.ReadMap())
                        {
                            ...
                            //if (Monster.MapID == 1020 || Monster.MapID == 1011 || Monster.MapID == 1000)
                            //{
                            //    if (Monster.SpawnCount > 5)
                            //    {
                            //        if (Monster.MapID == 1020)
                            //        {
                            //            Monster.SpawnCount = Monster.SpawnCount * 3;
                            //            Monster.MaxSpawnX += 20;
                            //            Monster.MaxSpawnY += 20;
                            //        }
                            //        else
                            //            Monster.SpawnCount = Monster.SpawnCount * 2;
                            //    }
                            //}
                            if (Monster.ID == 18)
                                Monster.SpawnCount *= 2;
